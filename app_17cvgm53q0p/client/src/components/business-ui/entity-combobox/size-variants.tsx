...
export const addButtonVariants = cva(
  [
    ...
    'data-[state=open]:bg-primary/25 dark:data-[state=open]:bg-primary/30',
  ],
  {
    variants: {
      size: {
...
/**
 * Button 模式下未选择时的空态选择框样式（长条、带边框，占位文字在按钮内）
 * medium: 40px，small: 32px，xs: 28px
 */
export const emptyTriggerVariants = cva(
  [
    'flex w-full justify-start gap-2 font-normal text-muted-foreground',
  ...
  ],
  {
    variants: {
      size: {
        medium: 'min-h-10 rounded-md px-3 text-sm', // 40px
        small: 'min-h-8 rounded-md px-3 text-sm', // 32px
        xs: 'min-h-7 rounded-md px-2 text-xs', // 28px
      },
    },
    defaultVariants: {
      size: 'medium',
